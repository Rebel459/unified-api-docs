...
You'll also notice that we call `NeoForgeUnifiedRegistries.registerBus` before we initialise the registries - this is done to ensure the deferred registers are setup before they are called. For more information on Unified API's multiloader registries & how NeoForge handles its registries, please refer [here](/unified-registries).